...
(function (jQuery) {
	...
	var Chart = function(div, opts) {
		...
		function render() {
            ...
            toggleProjects(opts.data, opts.toggleProjects);
		}
        ...
        function addGrid(div, data, dates, cellWidth, showWeekends) {
            ...
            for (var i = 0; i < data.length; i++) {
                gridDiv.append(jQuery("<div>", { "class": "ganttview-grid-item" }));
                for (var j = 0; j < data[i].series.length; j++) {
                    gridDiv.append(rowDiv.clone().addClass('item-id__' + i));
                }
            }
            div.append(gridDiv);
        }
        ...
        function addBlockContainers(div, data) {
            ...
            for (var i = 0; i < data.length; i++) {
                blocksDiv.append(jQuery("<div>", { "class": "ganttview-block-item" }));
                for (var j = 0; j < data[i].series.length; j++) {
                    blocksDiv.append(jQuery("<div>", { "class": "ganttview-block-container item-id__" + i }));
                }
            }
        ...
        function toggleProjects(data, toggleProjects) {
            $('div.ganttview-vtheader-item-name',div).on('click', function() {
                $thisId = $(this).attr('id');
                if(!$(this).hasClass('projectHidden')) {
                    $(this).addClass('projectHidden').next('div').hide();
                    $('.'+$thisId).hide();
                } else {
                    $(this).removeClass('projectHidden').next('div').show();
                    $('.'+$thisId).show();
                }
            })
        }
