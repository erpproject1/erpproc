...
function getLanguage(a){
     ...
     lang=data;
     $('.title h1').text(lang.employee.title);
     $('#btnCreateNew').text(lang.employee.button1);
     $('#idtable thead tr th:nth-of-type(2)').text(lang.employee.t_no);
     $('#idtable thead tr th:nth-of-type(3)').text(lang.employee.t_CompanyId);
     $('#idtable thead tr th:nth-of-type(4)').text(lang.employee.t_Type);
     $('#idtable thead tr th:nth-of-type(5)').text(lang.employee.t_Name); 
     $('#idtable thead tr th:nth-of-type(6)').text(lang.employee.t_MName);
     $('#idtable thead tr th:nth-of-type(7)').text(lang.employee.t_LName);
     $('#idtable thead tr th:nth-of-type(8)').text(lang.employee.t_NationalId);
     $('#idtable thead tr th:nth-of-type(9)').text(lang.employee.t_PassportNo);
     $('#idtable thead tr th:nth-of-type(10)').text(lang.employee.t_Country);
	 });
}
			...
			"aoColumnDefs": [
				{
                    ...
                    "mRender": function (data, type, full) {
                        return '<div class="btn-group btn-group-xs" role="group"> <a href="personel?EmpId='+data.id+'&_c=a" class="btn btn-default btn-icon btn-xs"><i class="fa fa-search"></i></a> <a href="personel?EmpId='+data.id+'" class="btn btn-default btn-icon btn-xs"><i class="fa fa-pencil"></i></a> </div> ';
                    }
                },
				{
                    "aTargets": [9],
                    "mRender": function (data, type, full) {
                     return  getCountry(data.nationalityId);
                    }
                },
                {
                    "aTargets": [10],
                    "mRender": function (data, type, full) {
                        return '<button  type="button" onclick="deletePersonal(this,'+data.id+')" class="btn btn-default btn-icon"><span class="fa fa-times"></span></button> ';
                    }
                }
              ],
              "language": {
            	    "url": lang.employee.datatableLang
            	  }
	...
	$.ajax({ type : "GET",
				...
				success : function(data) {
					...
					$(data).each(function(i, val) { 
										listBody = listBody + '<tr><td>'
												+'<div class="btn-group btn-group-xs" role="group"> <a href="personel?EmpId='+val.id+'&_c=a" class="btn btn-default btn-icon btn-xs"><i class="fa fa-search"></i></a> <a href="personel?EmpId='+val.id+'" class="btn btn-default btn-icon btn-xs"><i class="fa fa-pencil"></i></a> </div> '
												+ '</td><td>'
												+ val.id
												+ '</td><td>'
												+ val.companyId
												...
												+ val.passportNo
												+ '</td><td>'
												+ getCountry(val.nationalityId) 
												+ '</td><td>'
												+'<button  type="button" onclick="deletePersonal(this,'+val.id+')" class="btn btn-default btn-icon"><span class="fa fa-times"></span></button> ';
												+ '</td></tr>'
												 
									});
 
					$("#listEmployees").html(listBody);
				},
				error : function(data) {
					alert("Error! listEmployees");
				}
...
function deletePersonal(ctl,pId)
{
	_row = $(ctl).parents("tr");
